...
export const challengesRelations = relations(challenges, ({many, one}) => ({
  ...
  challegeProgress: many(challegeProgress)
}))
...
export const challegeProgress = pgTable('challenge_progress', {
  id: serial('id').primaryKey(),
...
export const challegeProgressRelations = relations(challegeProgress, ({one}) => ({
  challenge: one(challenges, {
    fields: [challegeProgress.challengeId],
    references: [challenges.id],
  })
}))
